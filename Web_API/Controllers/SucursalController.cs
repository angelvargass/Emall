...
namespace Web_API.Controllers
{
    ...
    public class SucursalController : ControllerBase {
        ...
        [HttpGet]
        public List<Regla> GetBySucursalId(int id) {
            return new SucursalManagement().ObtenerReglasPorSucursal(id);
        }

        [HttpPost]
        public IActionResult CrearRegla(Regla regla) {
            new SucursalManagement().CrearRegla(regla);

            return Ok();
        }

        [HttpPut]
        public IActionResult ModificarRegla(Regla regla) {
            new SucursalManagement().ModificarRegla(regla);
            return Ok();            
        }

        [HttpDelete]
        public IActionResult EliminarRegla(int id) {
            new SucursalManagement().EliminarRegla(id);
            return Ok();
        }
    }
}
